...
def registrar_venta(producto_id, cantidad_vendida, metodo_pago, usuario_id=None):
    fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # 1. Obtener datos del producto
    res = ejecutar_query("SELECT nombre, precio FROM productos WHERE id=?", (producto_id,), fetch=True)
    ...
    for i_id, i_nombre, stock_actual, cant_receta, unidad in insumos_receta:
        ...
        res_c = ejecutar_query("SELECT costo_unitario_base FROM inventario WHERE id=?", (i_id,), fetch=True)
    ...
    # 4. Procesar transacción con TRAZABILIDAD DE USUARIO
    try:
        ejecutar_query(
            """INSERT INTO ventas (fecha, producto_id, cantidad, total, metodo_pago, costo_historico_total, precio_historico_unitario, usuario_id) 
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (fecha, producto_id, cantidad_vendida, total_ingreso, metodo_pago, costo_total_momento, precio_actual, usuario_id)
        )

        for insumo_id, cant_desc in items_a_descontar:
            inventario_service.registrar_movimiento(
                insumo_id, 'salida', cant_desc, f"Venta {nombre_prod}", usuario_id
            )
